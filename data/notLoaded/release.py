...
lastmoded_file = os.path.join(directory,black_list_file[-1])
if os.path.exists(os.path.join(directory,black_list_file[-2])):
    t = ""
    with open(os.path.join(directory,black_list_file[-2]),"r") as f:
        t = f.read()
    upd_data = []
    for i in t.split("\n"):
        if i.startswith("#"):
...
update_small_pack = os.path.join(directory, "updates", "upd_%d.zip" % version)
update_full_pack  = os.path.join(directory, "updates", "upd_full.zip")

with open("cache/temp.php","w") as f:
...
if len(z.infolist()) > 0:
    ...
    ftp.storbinary("STOR update_info.php",open("cache/temp.php","rb"))
    ftp.quit()
    ...
    with open(os.path.join(directory,black_list_file[-2]),"w") as f:
